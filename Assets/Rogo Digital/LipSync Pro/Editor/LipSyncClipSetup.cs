...
#pragma warning disable 618
	...
	private const float version = 1.45f;
	private const string versionNumber = "Pro 1.45";
	...
	void OnAutoSyncDataReady (AudioClip clip, List<PhonemeMarker> markers)
	{
		...
		if (markers != null && markers.Count > 0)
		{
			...
			{
				markers[i].intensity = defaultPhonemeIntensity;
				markers[i].useRandomness = defaultUseRandomness;
				markers[i].intensityRandomness = defaultIntensityRandomness;
				markers[i].blendableRandomness = defaultBlendableRandomness;
				markers[i].bonePositionRandomness = defaultBonePositionRandomness;
				markers[i].boneRotationRandomness = defaultBoneRotationRandomness;
			}
